...
# --- 3. MODEL LOADING (DENGAN PROTEKSI ERROR) ---
@st.cache_resource
def load_sentiment_model():
    try:
        ...
        # Biar nggak malu pas di SS, kalau gagal load kita return None aja tanpa nampilin error merah
...
def get_stopwords():
    return set(['yang', 'dan', 'di', 'ke', 'dari', 'ini', 'itu', 'untuk', 'dengan', 'adalah', 'pada'])
...
def get_prediction(text):
    ...
    # Simulasi hasil HANYA jika model GAGAL load (buat screenshot)
    return "Positif", "😀", 98
...
# --- 8. DATA MANAGEMENT PAGE ---
    ...
    with st.container(border=True):
        ...
        if uploaded_file:
            st.markdown("<b><span class='label-skripsi'>c</span> Pratinjau Data</b>", unsafe_allow_html=True)
            st.info(f"File {uploaded_file.name} terbaca.")
            st.markdown("<b><span class='label-skripsi'>d</span> Kontrol Batch Analysis</b>", unsafe_allow_html=True)
...
# --- 9. SENTIMENT PREDICTION PAGE ---
    ...
    with st.container(border=True):
        ...
        input_text = st.text_area("", placeholder="Contoh: Aplikasi ini sangat membantu dalam belajar...", height=150, label_visibility="collapsed")
        
        st.write("")
        st.markdown("<b><span class='label-skripsi'>c</span> Tombol Eksekusi</b>", unsafe_allow_html=True)
