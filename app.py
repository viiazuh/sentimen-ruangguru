...
# Sistem akan mengecek apakah firebase sudah jalan. Kalau belum, dia pakai file json.
if not firebase_admin._apps:
    try:
        cred = credentials.Certificate("firebase-key.json")
        firebase_admin.initialize_app(cred)
    except Exception as e:
        st.error(f"Gagal menghubungkan ke Firebase: Pastikan file firebase-key.json ada di folder. Error: {e}")
...
# ==========================================
# HALAMAN 2: DATA MANAGEMENT (Lokal Saja)
# ==========================================
        ...
        if st.session_state.uploaded_df is not None:
            ...
            if run_analysis:
                with st.spinner("Sedang menganalisis dataset..."):
                    ...
                    progress_bar = st.progress(0, text="Preprocessing teks...")
                    ...
                    progress_bar.progress(0.5, text="Prediksi model (batch)...")
    ...
    if st.session_state.dataset is not None:
        with st.container(border=True):
            ...
            with c1: st.download_button("⬇️ CSV", csv_data, "hasil_sentimen.csv", "text/csv")
            with c2:
                if excel_ok:
                    st.download_button("⬇️ Excel", excel_data, "hasil_sentimen.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            with c3:
...
# ==========================================
# HALAMAN 3: SENTIMENT PREDICTION (Simpan ke Firebase)
# ==========================================
    ...
    with st.container(border=True):
        ...
        if st.button("Analisis Sentimen Sekarang"):
            if input_text:
                if model_ml is None:
                    st.error("Model tidak ditemukan! Pastikan folder 'models/' sudah benar.")
                else:
                    ...
                    if db:
                        try:
                            ...
                            st.warning(f"Hasil berhasil diproses, tapi gagal simpan ke Firebase: {e}")
