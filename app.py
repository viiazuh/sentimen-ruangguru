...
def load_sentiment_model():
    # Pastikan path ini sesuai dengan folder project Anda
    try:
...
def get_stopwords():
    return set([
        'yang', 'dan', 'di', 'ke', 'dari', 'ini', 'itu', 'untuk'import streamlit as st
import pandas as pd
import time
import re
import joblib
        ...
        if st.session_state.uploaded_df is not None:
            ...
            if run_analysis:
                with st.spinner("Sedang menganalisis dataset..."):
                    ...
                    progress_bar.progress(0.4, text="Tokenisasi & padding...")
                    ...
                    st.success(f"✅ Selesai! {len(texts_asli)} data diproses.")
...
# --- PAGE: SENTIMENT PREDICTION ---
    ...
    with st.container(border=True):
        input_text = st.text_area("Masukkan teks ulasan", placeholder="Contoh: Aplikasi ini sangat membantu belajar saya!", height=150)
        ...
        if st.button("Analisis Sentimen Sekarang"):
            if input_text.strip():
                ...
                st.warning("Silakan ketikkan ulasan terlebih dahulu."), 'dengan',
        'adalah', 'pada', 'juga', 'dalam', 'ada', 'tidak', 'saya', 'kami',
        'kita', 'mereka', 'akan', 'sudah', 'bisa', 'karena', 'lebih', 'atau',
        'tapi', 'kalau', 'jika', 'maka', 'sangat', 'sekali', 'saja', 'aja',
        'nya', 'lah', 'deh', 'dong', 'nih', 'sih', 'ya', 'yg', 'dgn', 'utk',
